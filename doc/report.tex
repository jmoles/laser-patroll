...
This work discusses a benchmarking of four sort algorithms tested against each other for a Portland State University ECE 588 project. The four sorts tested were C++'s \texttt{\texttt{std::sort}}, Quicksort, Merge sort, and Bitonic sort.
\end{abstract}
...
%TODO: Expand \texttt{std::sort} description. It apparently uses a introspective sort and heapsort, maybe? Should cite.
Basic sort was implemented by simply passing the data array to \texttt{\texttt{std::sort}}. In no case was it executed in a parallel fashion.
...
It is important to note that throughout all of these tests, anything labeled as ``basic'' is \texttt{\texttt{std::sort}} running in a \textbf{serial} fashion. We did not wrap any parallelization around \texttt{\texttt{std::sort}} so that each of the algorithms are evaluated against the sort that would likely be used in C++.
